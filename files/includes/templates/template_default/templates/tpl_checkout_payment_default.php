...
<?php // ** BEGIN PAYPAL EXPRESS CHECKOUT **
    ...
    <?php // BOF ZenExpert - POS set order status ?>
    <fieldset>
        <legend><?php echo TEXT_POS_ORDER_STATUS; ?></legend>
        <div class="pos-order-status">
            <?php
            $pos_statuses = explode(',',POS_ORDER_STATUS_SELECTION);
            foreach($pos_statuses as $pos_status) {
                echo '<div class="pos-status">'.zen_draw_radio_field('pos-status', $pos_status, '', 'id="pos-status-'.$pos_status.'"') . '<label class="radioButtonLabel" for="pos-status-'.$pos_status.'">' . zen_get_order_status_name($pos_status) . '</label></div>';
            }
            ?>
        </div>
    </fieldset>
    <?php // EOF ZenExpert - POS set order status ?>
