...
<!-- All HEADER_ definitions in the columns below are defined in includes/languages/english.php //-->
  <div class="row">
    ...
    <div class="col-xs-6 col-sm-3 col-sm-pull-3 noprint adminHeaderAlerts">
        ...
        ?>
<!-- BOF ZX POS login button -->
	<?php
//	$pos_sql = 'SELECT admin_name, admin_email from ' . TABLE_ADMIN . ' WHERE admin_id = :adminId: AND admin_profile IN (' . POS_LOGIN_ADMIN_PROFILE_ID . ')';
	$pos_sql = 'SELECT admin_name, admin_email from ' . TABLE_ADMIN . ' WHERE admin_id = :adminId: AND admin_profile IN (1,2)';
    $pos_sql = $db->bindVars($pos_sql, ':adminId:', $_SESSION['admin_id'], 'integer');
    $pos_result = $db->Execute($pos_sql);
	$pos_token = md5($pos_result->fields['admin_name'].$pos_result->fields['admin_email']);
	echo '<div style="float:right;"><form target="_blank" name="login" action="' . zen_catalog_href_link(FILENAME_BARCODE_CART, '', 'SSL') . '" method="post">' . zen_draw_hidden_field('POStoken', $pos_token) . zen_image_submit('button_pos_login.gif', POS_BUTTON_ADMIN_LOGIN_ALT) . '</form></div>'; ?>
    <!-- EOF ZX POS login button -->
    </div>
