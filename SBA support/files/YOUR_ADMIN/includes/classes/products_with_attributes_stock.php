...
/*          $query = 'SELECT * FROM ' . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . ' WHERE products_id="' . $products->fields['products_id'] . '"
          ...
          if ($attribute_products->RecordCount() > 0) {
              ...
              $html .= '<th class="stockAttributesHeadingStockId">' . PWA_STOCK_ID.'</th>
                    ...
                    <!-- BOF ZX POS 1/17 -->
                    <th class="stockAttributesHeadingCustomid">'.PWA_BARCODE.'</th>
                    <!-- EOF ZX POS 1/17 -->
...
//                  sort($attributes_output);
		  ...
		  // BOF ZX POS 2/17
                  $html .= '<td class="stockAttributesCellBarcode editthis" id="stockid-barcode-'. $attribute_products->fields['stock_id'] .'">'.$attribute_products->fields['barcode'].'</td>'."\n";
		  // EOF ZX POS 2/17
                  $html .= '<td class="stockAttributesCellEdit">' . "\n";
                  // BOF ZX POS 3/17
                  $html .= '<a href="'.zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, "action=edit&amp;products_id=".$products->fields['products_id'].'&amp;attributes='.$attribute_products->fields['stock_attributes'].'&amp;q='.$attribute_products->fields['quantity'] .'&amp;barcode='.$attribute_products->fields['barcode'] . '&amp;search_order_by=' . $search_order_by, 'NONSSL').'">'.PWA_EDIT_QUANTITY.'</a>'; //s_mack:prefill_quantity
                  // EOF ZX POS 3/17
                  $html .= '</td>' . "\n";
...
function insertNewAttribQty($products_id = null, $productAttributeCombo = null, $strAttributes = null, $quantity = 0, $customid = null, $skuTitle = null, $barcode = null){
// EOF ZX POS 5/17
  global $db;
  ...
  if (isset($products_id) && is_numeric($products_id) && isset($strAttributes) && is_numeric($quantity)) {
      ...
      // BOF ZX POS 6/17
      if (!isset($barcode) || trim($barcode) == '' || trim($barcode) == "''" || $barcode === 'null') {
          $barcode = null;
      }
      $barcode = $this->nullDataEntry($barcode);
      // EOF ZX POS 6/17
      ...
      // BOF ZX POS 7/17
      if (!isset($barcode)) {
        $query = $db->bindVars($query, ':barcode:', 'null', 'noquotestring');
      } else {
        $query = $db->bindVars($query, ':barcode:', $barcode, 'string'); // @TODO Need to also consider ignore NULL style.
      }
      // EOF ZX POS 7/17
	  ...
	  // BOF ZX POS 8/17
          $query = "INSERT INTO " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . " (`products_id`, `product_attribute_combo`, `stock_attributes`, `quantity`, `customid`, `title`, `barcode`) 
           values (:products_id:, :product_attribute_combo:, :stock_attributes:, :quantity:, :customid:, :title:, :barcode:)";
	   // EOF ZX POS 8/17
          $query = $db->bindVars($query, ':products_id:', $products_id, 'integer');
	  ...
	  // BOF ZX POS 9/17
          if (!isset($barcode)) {
            ...
            $query = $db->bindVars($query, ':barcode:', $barcode, 'passthru'); // @TODO Need to also consider ignore NULL style.
          }
	  // EOF ZX POS 9/17
	      ...
	      // BOF ZX POS 10/17
              $query = "UPDATE " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . " set `quantity` = :quantity:, `customid` = :customid:, `title` = :title:, `barcode` = :barcode: WHERE `products_id` = :products_id: AND `stock_attributes` = :stock_attributes:";
	      // EOF ZX POS 10/17
              $query = $this->query_insert_float($query, ':quantity:', $quantity);
              if (!isset($customid)) {
                $query = $db->bindVars($query, ':customid:', 'null', 'noquotestring');
...
//The on duplicate updates an existing record instead of adding a new one
// BOF ZX POS 12/17
function insertTablePASR($products_id = null, $strAttributes = null, $quantity = null, $customid = null, $barcode = null){
// EOF ZX POS 12/17
  ...
  $barcode = $this->nullDataEntry($barcode);//sets proper quoting for input
  // EOF ZX POS 13/17
  ...
  if (is_numeric($quantity) && is_numeric($pasrid)) {
    ...
    // BOF ZX POS 14/17
    $query = "insert into ". TABLE_PRODUCTS_ATTRIBUTES_STOCK ." (`quantity`,`customid`,`barcode`)
          values ($quantity, $customid, $barcode)
          ON DUPLICATE KEY UPDATE
          `quantity` = $quantity,
          `customid` =  $customid,
          `barcode` = $barcode;";
...
//The on duplicate updates an existing record instead of adding a new one
// BOF ZX POS 15/17
function insertTablePAS($products_id = null, $quantity = null, $customid = null, $barcode = null){
// EOF ZX POS 15/17
  ...
  // BOF ZX POS 16/17
  $barcode = addslashes($barcode);
  $barcode = $GLOBALS['db']->prepare_input($barcode);
  $barcode = $this->nullDataEntry($barcode);//sets proper quoting for input
  // EOF ZX POS 16/17
...
// BOF ZX POS 17/17
    $query = "insert into ". TABLE_PRODUCTS_ATTRIBUTES_STOCK ." (`products_id`,`quantity`,`customid`,`barcode`)
