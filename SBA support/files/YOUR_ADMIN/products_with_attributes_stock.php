...
// If doesn't exist then need to go back to add, if does exist then need to go to update.
      ...
      if (isset($_POST['barcode']) && zen_not_null($_POST['barcode'])) {
      ...
      // BOF ZX POS 2/
      $hidden_form .= zen_draw_hidden_field('barcode', $barcode) . "\n";
      // EOF ZX POS 2/
      ...
      // BOF ZX POS 3/
      $s_mack_noconfirm .= "barcode=" . $barcode . "&"; //s_mack:noconfirm
      // EOF ZX POS 3/
...
/*    if (isset($_GET['customid']) && $_GET['customid']) {
    ...
    $barcode = zen_db_prepare_input($barcode);
    $barcode = zen_db_input($barcode);
    // EOF ZX POS 4/
...
/*        foreach ($arrAttributes as $arrAttrKey => $arrAttrVal) {
        ...
        $productAttributeCombo = $products_id . '-' . str_replace(',', '-', $strAttributes);
	// BOF ZX POS 5/
        $saveResult = $stock->insertNewAttribQty($products_id, $productAttributeCombo, $strAttributes, $quantity, $customid, $skuTitle, $barcode);
	// EOF ZX POS 5/
      }
      ...
      // BOF ZX POS 6/
      if (!empty($barcode)) {
        $saveResult = $stock->updateBarcodeAttrib($stock_id, $barcode);
      }
      // EOF ZX POS 6/
    }
